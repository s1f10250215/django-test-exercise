...
class TodoViewTestCase(TestCase):
    ...
    def test_index_get_refine_completed(self):
        task1 = Task(title='task1', due_at=timezone.make_aware(datetime(2024, 7, 1)), completed=True)
        task1.save()
        task2 = Task(title='task2', due_at=timezone.make_aware(datetime(2024, 8, 1)), completed=False)
        task2.save()
        client = Client()
        response = client.get('/?refine=completed')
    ...
    def test_index_get_refine_not_completed(self):
        task1 = Task(title='task1', due_at=timezone.make_aware(datetime(2024, 7, 1)), completed=True)
        task1.save()
        task2 = Task(title='task2', due_at=timezone.make_aware(datetime(2024, 8, 1)), completed=False)
        task2.save()
        client = Client()
        response = client.get('/?refine=not_completed')
